...
aws ecr describe-repositories --repository-names ${METRIC_APP_NAME} 2>/dev/null || \
aws ecr create-repository --repository-name ${METRIC_APP_NAME}
...
aws ecs create-cluster \
    --cluster-name $METRIC_CLUSTER_NAME \
...
aws logs create-log-group --log-group-name "/ecs/${METRIC_APP_NAME}" 2>/dev/null || true
